...
class LeaveTests(TestCase):
    def setUp(self):
        ...
        self.hr_admin = get_user_model().objects.create_user(username='hr', password='pass', role='HR_ADMIN')
        self.client = APIClient()

    def test_employee_can_apply_leave(self):
        self.client.force_authenticate(user=self.employee)
        resp = self.client.post('/api/leaves/apply/', {
            'leave_type': 'SICK',
    ...
    def test_manager_can_view_pending_leaves(self):
        self.client.force_authenticate(user=self.employee)
        self.client.post('/api/leaves/apply/', {
            'leave_type': 'SICK',
            'start_date': '2026-04-01',
            'end_date': '2026-04-02',
            'reason': 'Feeling unwell'
