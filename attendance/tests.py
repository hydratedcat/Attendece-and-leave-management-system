...
class AttendanceTests(TestCase):
    ...
    def test_employee_can_view_own_attendance(self):
        self.client.force_authenticate(user=self.employee)
        Attendance.objects.create(user=self.employee, date=date.today(), status='PRESENT')
    ...
    def test_monthly_attendance_report(self):
        ...
        Attendance.objects.create(user=self.employee, date=today - timedelta(days=30), status='ABSENT')
